...
import com.prismmods.dwdimensions.api.GravityHandler;
...
public class GravityManager {
    ...
    public static void jumpGravityModifier(LivingEvent.LivingJumpEvent event) {
        ...
        if(!GravityHandler.shouldUseNormalGravity(entity)) {
        ...
        if(!GravityHandler.shouldUseNormalGravity(entity)) {
            if (entity.getLevel().dimension() == DWDDimensionReg.SPACE_KEY) {
                if (!entity.isOnGround()) {
                    Vec3 motion = entity.getDeltaMovement();
                    if (entity instanceof Player) {
                        if (((Player) entity).getAbilities().flying || (entity.isFallFlying())) {
                            //ignore
                        } else {
